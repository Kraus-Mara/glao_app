	...
	def _count_stock_in_spie(self):
		if self.article_referenced:
			places_stock = frappe.get_all(
				"Places Stock",
				filters=[
					["parent", "like", str(self.article_referenced)],
					["external", "=", 0],
				],
			...
			all_stocks_not_registered = frappe.get_all(
				"Stock",
				filters=[
					["article", "like", str(self.article_to_register)],
			...
			if all_stocks_not_registered:
				...
				frappe.db.set_value(
					"Stock",
					str(all_stocks_not_registered[0].name),
					"quantity_in_spie_tm",
					qty,
				)
	...
	def _creer_stock_entry(self):
		...
		if not existing:
			...
			all_ps = frappe.get_all(
				"Places Stock",
				filters=[["parent", "=", existing[0].name]],
				fields="quantity",
			)
		...
		for doc in self.placetostock:
			try:
				...
				existing = frappe.get_all(
					"Places Stock",
					filters=[
						["parent", "like", self.article],
						["place", "like", doc.place],
					],
				)
				if existing:
	...
	def _transfert_normal(self):
		...
		if source:
			...
			if new_qty < 0:
				...
				places_stock = frappe.get_all(
					"Places Stock",
					filters=[["parent", "like", self.article_from_stock]],
					fields=["quantity"],
				)
